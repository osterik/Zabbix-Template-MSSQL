...
* Триггер "Access Methods Page Splits / Sec" 
...
# Как сделать шаблон для Express редакции (или для случая, когда в системе установлено несколько экземпляров MSSQL)
  ...
  4. Открыть экспортированный .xls в текстовом редакторе и заменить все вхождения "SQLServer:" на "MSSQL$SQLEXPRESS:". Если создаётся шаблон для экземпляра MSSQL с нестандарным именем - указать это имя. Посмотреть его можно при добавлении счётчиков в perfmon.exe
